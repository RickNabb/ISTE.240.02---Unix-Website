function validateQuestions() {
		...
		var answersRight = 0;
		// subtract 1 from answers.length to account for additional input tag in embedded unix terminal
		var numQuestions = (answers.length-1)/4;
		
		// loop through all input besides submit button (last input element)
		for (var i=0; i<answers.length; i++){
			// reset values
			answers[i].parentNode.parentNode.style.backgroundColor="transparent";
